...
Hybridly has sensible defaults for controlling the version. It looks for a manifest generated by Vite or by the `app.asset_url` option in case the application is hosted on Vapor.

This logic can be extended or overriden by extending the `version` method of the middleware.

Additionally, calling `hybridly()->resolveVersionUsing(fn () => $version)` will override the version for the next request—this can be used to forcefully invalidate the next response.
