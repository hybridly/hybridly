...
The `\Hybridly\Hybridly::DEFAULT_ROOT_VIEW` constant has been moved to `Hybridly\Configuration\Architecture::ROOT_VIEW` and will need to be replaced in `config/hybridly.php` as well as other places it may be used.

```php
use Hybridly\Hybridly; // [!code --] // [!code focus:2]
use Hybridly\Configuration\Architecture; // [!code ++]
