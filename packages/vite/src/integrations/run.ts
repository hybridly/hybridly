...
async function getRunOptions(options: ResolvedOptions): Promise<Runner[]> {
	...
	return [
		{
			...
			run: [
				php,
				path.resolve(options.laravelPath, 'artisan'),
				'hybridly:types',
		...
		{
			name: 'Generate i18n',
			run: [php, path.resolve(options.laravelPath, 'artisan'), 'hybridly:i18n'].flat(),
			pattern: 'lang/**/*.php',
		},
		...options.run ?? [],
	]
