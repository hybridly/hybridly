...
export function setHistoryState(options: HistoryOptions = {}) {
	if (!window?.history) {
		throw new Error('The history API is not available, so Monolikit cannot operate.')
	}
	...
	try {
		...
		console.error('Monolikit could not save its current state in the history. This is most likely due to a property being non-serializable, such as a proxy or a reference.')
		throw error
	}
}
...
export async function registerEventListeners() {
	...
	window?.addEventListener('popstate', async(event) => {
		...
		// If the history entry has been monolikitly tempered with, we want
