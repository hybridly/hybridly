...
import qs from 'qs'
import { showResponseErrorModal, match, merge, when, debug } from '@monolikit/utils'
	...
	try {
		...
		const response = await axios.request({
			...
			headers: {
				...
				// 'X-Monolikit-Context': this.currentState.visit.context,
				[MONOLIKIT_HEADER]: true,
		...
		if (!isMonolikitResponse(response)) {
...
/** Checks if the response contains a monolikit header. */
export function isMonolikitResponse(response: AxiosResponse): boolean {
	return !!response?.headers[MONOLIKIT_HEADER]
}
