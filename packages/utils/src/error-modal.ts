class Modal {
	...
	initializeDOM() {
		...
		if (document.querySelector('[data-monolikit-overlay="true"]')) {
			return false
		}
		...
		style.innerHTML = `
			...
			[data-monolikit="visible"] {
				opacity: 1;
			}

			[data-monolikit] iframe {
	...
	show() {
		...
		this.overlay.dataset.monolikit = 'visible'
	}

	destroy() {
		this.overlay.dataset.monolikit = ''
		setTimeout(() => {
